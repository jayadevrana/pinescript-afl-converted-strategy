...
This repository contains a Pine Script source file saved as `pinescript_afl_converted_strategy.txt` so it opens easily on GitHub and can be copied into TradingView.
...
AFL Converted Strategy gives traders a practical chart workflow around EMA, SMA, RSI, Supertrend. The source is kept readable in a single `.txt` file so the logic can be reviewed, copied, and adapted inside TradingView.
...
3. Copy the code from `pinescript_afl_converted_strategy.txt`.
